...
FriendlyEats.prototype.viewList = function(filters, filter_description) {
  ...
  that = this;
  headerEl.querySelector('#logout-button').addEventListener('click', function(event) {
    that.stopAllListeners();    
...
FriendlyEats.prototype.initReviewDialog = function() {
  ...
  dialog.querySelectorAll('.star-input span').forEach(function(el) {
    var rate = function() {
      var after = false;
      rating = 0;
      [].slice.call(el.parentNode.children).forEach(function(child) {
        let curr = child.isSameNode(el);
        if (!after) {
          rating++;
      ...
      that.dialogs.add_review.getDefaultFoundation().accept(true);
    };
    ...
    .then(function(ratings) {
      ...
      if (ratings.size) {
        ...
        ratings.forEach(function(rating) {
          ...
          el.querySelector('.timestamp').append(data.timestamp.toDate().toLocaleDateString("en-US"))
          mainEl.querySelector('#cards').append(el);
        });
      } else {
...
FriendlyEats.prototype.renderRating = function(rating) {
  ...
  const moodContent = this.renderTemplate('mood-icon', {'mood': moods[rating - 1]});
  el.append(moodContent);
